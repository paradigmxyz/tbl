...
use tabl_core::formats::{print_bullet, print_header};
...
pub(crate) async fn print_summary(
    ...
    let n_input_bytes = tabl_core::filesystem::get_total_bytes_of_files(&all_input_files).await?;
...
fn print_input_summary(
    ...
    print_bullet(
        "n_input_bytes",
        tabl_core::formats::format_bytes(n_input_bytes),
    );
    print_bullet(
        "n_input_files",
        tabl_core::formats::format_with_commas(n_input_files as u64),
    );

    let n_show_files = 10;
    for path in input_files.iter().take(n_show_files) {
        let path: String = path.to_string_lossy().to_string();
        tabl_core::formats::print_bullet_key_indent(path, 4);
    }
    if input_files.len() > n_show_files {
        tabl_core::formats::print_bullet_key_indent("...", 4);
    }
}
