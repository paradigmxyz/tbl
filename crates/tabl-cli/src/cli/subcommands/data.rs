...
use tabl_core::filesystem::{get_input_paths, get_output_paths, OutputPathSpec};
...
fn gather_inputs_and_outputs(
    ...
    args: &DataArgs,
) -> Result<Vec<(Vec<PathBuf>, Option<PathBuf>)>, TablCliError> {
...
fn process_io(
    ...
    // create lazy frame
    let lf = tabl_core::parquet::create_lazyframe(&input_paths)?;
